...
- **Football highlights, spoiler-free (2026-09-12)** · `src/lib/news/highlights.ts`: five YouTube channel feeds (beIN SPORTS Arabic → CL/La Liga/PL/Ligue 1 · Serie A official → Serie A · FC Bayern, BVB, RB Leipzig → their Bundesliga games), titles parsed to `home/away/competition/context`, stored in table `highlights` (video_id unique, 21-day retention, same match from two sources deduped). Polled by the reminders tick every 5 min (`pollHighlights`, self-creates the table) and inline by `GET /api/highlights` when stale. News shows "Home vs Away" + context only, tap → YouTube app. Team map `TEAMS` (Arabic + Latin aliases); CL participants = `CL_TEAMS_SEED` + any team seen in a stored CL highlight. Known gap: VfB Stuttgart league games (their channel posts none).
