...
export function RepEditor({ exercise, showSets, kettlebellKg, onSave, onClose }: {
  ...
  return (
    <>
      ...
      >
        <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 6 }}>{exercise.name}</div>
        ...
        {exercise.kettlebell ? (
          <div style={{ fontSize: 15, color: "var(--ink-3)", minHeight: 40, display: "flex", alignItems: "center" }}>
        ...
        <button className="cc-btn cc-btn-primary" onClick={done} style={{ minHeight: 52, borderRadius: 14, fontSize: 17, marginTop: 8 }}>Done</button>
      </div>
    </>
  );
}
