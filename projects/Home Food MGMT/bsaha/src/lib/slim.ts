...
export function toSlim(d: Dish): SlimDish {
  return {
    id: d.id, slug: d.slug, meal: d.meal, meals: d.meals ?? [],
    categories: d.categories ?? [], foodGroups: (d.foodGroups ?? []).map((g) => g.group),
    nameEn: d.nameEn, nameFr: d.nameFr, nameAr: d.nameAr, nameLatin: d.nameLatin,
    cuisine: d.cuisine, inMain: d.inMain, lean: d.isLean, onMenu: d.onMenu, menuMeals: d.menuMeals ?? [], tags: d.tags ?? [],
    kcal: Math.round(d.macros?.kcal ?? 0), protein: Math.round(d.macros?.protein_g ?? 0),
    carbs: Math.round(d.macros?.carbs_g ?? 0), fiber: Math.round(d.macros?.fiber_g ?? 0),
    minutes: (d.prepMin ?? 0) + (d.cookMin ?? 0), rating: d.rating ?? null, photo: d.photoUrl,
 ...
 * Returns the whole library; screens filter by `onMenu` themselves.
 */
...
export async function listSlimDishes(): Promise<SlimDish[]> {
  ...
  const rows = await db
    .select({
      id: dishes.id, slug: dishes.slug, meal: dishes.meal, meals: dishes.meals,
      categories: dishes.categories, foodGroups: dishes.foodGroups, rating: dishes.rating,
      nameEn: dishes.nameEn, nameFr: dishes.nameFr, nameAr: dishes.nameAr, nameLatin: dishes.nameLatin,
      cuisine: dishes.cuisine, inMain: dishes.inMain, isLean: dishes.isLean, onMenu: dishes.onMenu, menuMeals: dishes.menuMeals, tags: dishes.tags, macros: dishes.macros,
