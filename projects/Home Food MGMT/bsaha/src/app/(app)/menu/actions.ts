...
import { DISLIKE_TAGS, MEALS, dishes, people, photos, picks, pools, type Meal } from "@/db/schema";
...
/** A `data:image/...;base64,` string from the form, decoded and checked. */
function decodePhoto(raw: unknown): { mime: string; bytes: Buffer } | null {
  if (typeof raw !== "string" || !raw) return null;
  const m = /^data:(image\/(?:jpeg|png|webp));base64,([A-Za-z0-9+/=]+)$/.exec(raw);
  if (!m) return null;
  const bytes = Buffer.from(m[2], "base64");
  if (bytes.length < 100 || bytes.length > MAX_PHOTO) return null;
  return { mime: m[1], bytes };
}

/** Saves a family photo and returns the URL to put on the dish. */
async function storePhoto(full: unknown, small: unknown, by: number): Promise<string | null> {
  const big = decodePhoto(full);
  if (!big) return null;
  const thumb = decodePhoto(small);
  const id = `${Date.now().toString(36)}${Math.random().toString(36).slice(2, 8)}`;
  await db.insert(photos).values({ id, mime: big.mime, bytes: big.bytes, thumb: thumb?.bytes ?? null, createdBy: by, createdAt: new Date().toISOString() });
  return `/api/photo/${id}`;
}
...
export async function addDish(formData: FormData) {
  ...
  const photoUrl = await storePhoto(formData.get("photo_full"), formData.get("photo_thumb"), me.id);
  ...
  if (photoUrl && row) await db.update(photos).set({ dishId: row.id }).where(eq(photos.id, photoUrl.slice("/api/photo/".length)));
  forgetSlimDishes();
  redirect(`/menu/${slug}`);
}
