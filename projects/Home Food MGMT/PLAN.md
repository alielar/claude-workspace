...
- **Library and menu are two layers (decided 2026-09-20, supersedes the Removed library).** The library holds every dish ever added (355 today). The menu is the hand-picked subset the family and the cook actually see, about 30 per meal, ticked on and off from the Library screen, which shows a per-meal count against that target, filters, protein sorting and a clear-this-meal button. Taking a dish off the menu leaves it in the library and drops it from shortlists and choices. Bulk imports land in the library only (`on_menu: false`); a re-seed never touches `on_menu`, so nothing chosen is ever undone by a deploy.
- **International library rebuilt from scratch (decided 2026-09-20, supersedes the NHS and MyPlate sets).** Ali wanted the library to look like the everyday sections of a big recipe site (Healthy, Quick, Easy, Weeknight, Pasta, by meal). NYT Cooking was checked as a source and rejected: its recipes and photos are paid, copyrighted content, its terms forbid copying them into another product or any automated collection, and its robots file blocks Anthropic's crawlers by name. So the site was used for nothing but the idea of the categories. 96 well-known dishes were written in-house to `data/RULES.md` (24 breakfast, 36 lunch, 36 dinner; halal, Morocco-sourceable, Darija steps, per-serving macros, a `pasta` tag for a future filter) in `data/dishes/intl-*.json`. They land off the menu; Ali picks the menu from them. The NHS and MyPlate files moved to `data/dishes-archive/nhs-myplate-2026-09-20/` and their 274 photos were removed; the stale sweep in the seeder deletes those dishes from the database on the next migrate. The Moroccan menu (68 dishes) and family custom dishes are untouched. Photos come from Wikimedia and Openverse (Flickr CC), up to six candidates per dish gathered by `scripts/photo-candidates2.ts` (now honours each dish's `photo_query` and an `ONLY_PREFIX` file filter) and chosen by eye from contact sheets; 94 of 96 have one, two (savoury oatmeal with egg, sweet potato and egg hash) show a letter tile until a better free photo or a kitchen photo turns up. Free-licence food photos are honest but rarely magazine-grade; any can be swapped from the dish page. NYT photos are not used even if downloaded by hand.
- **Quantities follow the number of people (2026-09-20).** Every recipe stores ingredients for its own `servings`. The dish page has a row of people counts (1, 2, 4, 6, 8, plus the recipe's own) and scales each line live, rounded to what the cook can measure: grams and millilitres to round numbers, pieces to halves, spoons to quarters shown as fractions. From the cook's orders screen a dish opens at the number of people who chose it; otherwise at the count last used on that device, else the recipe's own. The Darija steps are not rewritten; the ingredient list is the source of quantities. The nutrition panel follows the same count: it always shows the per-person estimate (a plate is the same size however many eat) and, above one person, a second row with the whole pot at that count, so the total protein cooked is visible. Macros stay stored per serving; there is no per-ingredient nutrition data, so scaling is proportional.
- **Menu emptied, then hand-built; delete for good from anywhere (decided 2026-09-20).** The whole menu was emptied once on deploy (a one-off keyed in a new `settings` table, so it never runs twice) and is now built by hand from the Library screen, which has an "Empty the whole menu" button next to the per-meal one. Any dish, on the menu or not, can be deleted for good from its library tile or its dish page, after one confirmation. A deleted dish keeps its row with `status = 'deleted'` as a tombstone: nothing in the app reads it, and the library refresh that runs after each deploy cannot bring it back because the upsert never touches `status`.
