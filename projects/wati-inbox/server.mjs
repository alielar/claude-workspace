...
import { db, inbox, getThread as storedThread, threadMessages, saveThread, latestSuggestion, sentTemplates, logSend, addSubscription, removeSubscription, subscriptions } from './db.mjs';
import { sendText, sendTemplate, frenchTemplates, getThread as liveThread } from './wati.mjs';
...
async function api(req, res, path) {
  ...
  if (path === '/api/inbox') {
    return json(res, 200, { threads: inbox().map((t) => ({ ...t, windowOpen: !!t.last_inbound_at && hoursSince(t.last_inbound_at) < 24 })) });
  }
  if (path === '/api/push') {
  ...
  const m = /^\/api\/thread\/(\d{8,15})(?:\/(send|template|handled|refresh))?$/.exec(path);
  ...
  if (!action && req.method === 'GET') {
    ...
    if (!t) return json(res, 404, { error: 'Aucune conversation lisible pour ce numéro' });
  ...
  if (action === 'send' && req.method === 'POST') {
    ...
    for (const text of bubbles) {
      try { await sendText(waId, text); sent.push(text); logSend(waId, 'text', { text }, true); }
  ...
  if (action === 'template' && req.method === 'POST') {
    ...
    if (t) saveThread({ ...t, pending: 0, last_outbound_at: new Date().toISOString(), last_text: `[${tpl.name}]` });
...
// ── server ───────────────────────────────────────────────────────────────────
const server = createServer({ key: readFileSync('certs/server.key'), cert: readFileSync('certs/server.pem') }, async (req, res) => {
...
server.listen(PORT, () => {
  console.log(`Wati Inbox on https://localhost:${PORT}  (phone: https://${process.env.LOCAL_HOST || 'Alis-MacBook-Pro.local'}:${PORT})`);
  startPolling();
});
