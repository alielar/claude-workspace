...
> Last updated: 2026-09-13 (vault backup/restore, keyboard-safe sheets, notes ≡ icon, Next week chip). Read `ALI_SPEC.md` first — it is the product brief and phase plan. This file is the engineering map.
...
src/app/(app)/stretch     "Mobility" (renamed from Stretching 2026-09-12, display text only · route, routineKey "stretch", localStorage keys unchanged): guided timer, 21 moves in 4 blocks, per-move 15-35s + 10s rests, exactly 12:00, wake lock, voice + beeps; renames stored by move key, never by position
